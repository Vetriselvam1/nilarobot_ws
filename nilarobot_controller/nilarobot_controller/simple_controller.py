...
class SimpleController(Node):
    ...
    def __init__(self):
        ...
        self.vel_sub_ = self.create_subscription(TwistStamped, "nilarobot_controller/cmd_vel", self.velCallback, 10)
        self.joint_sub_ = self.create_subscription(JointState,"joint_states", self.jointCallback, 10)        
        self.odom_pub_ = self.create_publisher(Odometry, "nilarobot_controller/odom", 10)
    ...
    def velCallback(self, msg):
        ...
        nilarobot_speed = np.array([[msg.twist.linear.x],
                                [msg.twist.angular.z]])
        wheel_speed = np.matmul(np.linalg.inv(self.speed_conversion_), nilarobot_speed) 
